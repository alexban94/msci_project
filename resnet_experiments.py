...
if __name__ == "__main__":
    ...
    # Experiment name: parameters (dataset, rotations, generated_ims, num_generated_ims)
    experiments = {
        "Face - None" : ["gestational_face", False, False, 0],
        "Face - Rotate" : ["gestational_face", True, False, 0],
        "Face - GAN" : ["gestational_face", False, True, 30],
        "Face - Rotate + GAN" : ["gestational_face", True, True, 30],

        "Ear - None" : ["gestational_ear", False, False, 0],
        "Ear - Rotate" : ["gestational_ear", True, False, 0],
        "Ear - GAN" : ["gestational_ear", False, True, 30],
        "Ear - Rotate + GAN" : ["gestational_ear", True, True, 30],

        "Foot - None" : ["gestational_foot", False, False, 0],
        "Foot - Rotate" : ["gestational_foot", True, False, 0],
        "Foot - GAN" : ["gestational_foot", False, True, 30],
        "Foot - Rotate + GAN" : ["gestational_foot", True, True, 30]
    }

    experiments_gan_ims = {

        "Face - GAN - 0" : ["gestational_face", False, False, 0],
        "Face - GAN - 15" : ["gestational_face", False, True, 15],
        "Face - GAN - 30" : ["gestational_face", False, True, 30],
        "Face - GAN - 45" : ["gestational_face", False, True, 45],
        "Face - GAN - 60" : ["gestational_face", False, True, 60],
        "Face - GAN - 120" : ["gestational_face", False, True, 120],
        
        "Ear - GAN - 0" : ["gestational_ear", False, False, 0],
        "Ear - GAN - 15" : ["gestational_ear", False, True, 15],
        "Ear - GAN - 30" : ["gestational_ear", False, True, 30],
        "Ear - GAN - 45" : ["gestational_ear", False, True, 45],
        "Ear - GAN - 60" : ["gestational_ear", False, True, 60],
        "Ear - GAN - 120" : ["gestational_ear", False, True, 120],
        
        "Foot - GAN - 0" : ["gestational_foot", False, False, 0],
        "Foot - GAN - 15" : ["gestational_foot", False, True, 15],
        "Foot - GAN - 30" : ["gestational_foot", False, True, 30],
        "Foot - GAN - 45" : ["gestational_foot", False, True, 45],
        "Foot - GAN - 60" : ["gestational_foot", False, True, 60],
        "Foot - GAN - 120" : ["gestational_foot", False, True, 120]
    ...
    for exp_name, config in experiments.items():
        data_dir = "/cs/home/psyajhi/data/" + config[0]
        gen_folders = sorted(glob2.glob(gen_dir + '/split_*_' + config[0]))

        # Include rotations/synthetic data?
        rotate_flag = config[1]
        gen_flag = config[2]
        num_gen_images = config[3]

        exp_overall_acc = []
        exp_overall_f1s = []
        # Run 5 times for the average.
        for i in range(5):
            ...
            report = classification_report(y_labels, y_pred, target_names=plot_labels, output_dict=True)
            print(report)
            f1 = []
            ...
            # Overall acc
            overall_acc = np.sum(y_labels == y_pred)/len(y_labels)

            # Store for average calculation
            exp_overall_acc.append(overall_acc)
            exp_overall_f1s.append(f1)
        ...
        exp_mean_acc = np.mean(exp_overall_acc)
        exp_std_acc = np.std(exp_overall_acc)
        exp_mean_f1 = np.mean(np.array(exp_overall_f1s), axis=0)
        exp_std_f1 = np.std(np.array(exp_overall_f1s), axis=0)

        means[exp_name] = np.concatenate((exp_mean_f1, [exp_mean_acc]))
        stds[exp_name] = np.concatenate((exp_std_f1, [exp_std_acc]))

    print(means)
    print(stds)
    
    cols = ("Extremely", "Very", "Moderate", "Term", "Late", "Overall accuracy")
    mean_df = pd.DataFrame.from_dict(means, orient='index', columns=cols)
    std_df = pd.DataFrame.from_dict(stds, orient='index', columns=cols)

    print(mean_df)
    print(std_df)
    mean_df.to_csv(out + '_mean_experiments.csv')
    std_df.to_csv(out + '_std_experiments.csv')
