...
from evaluation import sample_generate, sample_generate_conditional, sample_generate_light, calc_inception, calc_FID, calc_KID
...
def main():
    ...
    report_keys = ["loss_dis", "loss_gen", "loss_FM", 
                    "FID_min", "FID_mean", "FID_std", 
                    "KID_min", "KID_mean", "KID_std"]

    # Set up logging
    #trainer.extend(extensions.snapshot(filename='snapshot_best'),
    #               trigger=training.triggers.MinValueTrigger("KID_mean", trigger=(config.evaluation_interval, 'iteration')))
    ...
    trainer.extend(sample_generate_light(G_tgt, out, rows=10, cols=10),
                   ...
                   priority=extension.PRIORITY_WRITER)
    ## FID - added path to inception model.
    trainer.extend(calc_FID(G_tgt, n_ims=5000, stat_file=config['eval']['stat_file'], path='datasets/inception_model', num_evals=1),
                   trigger=(config.evaluation_interval, 'iteration'),
                   priority=extension.PRIORITY_WRITER)
    ## TODO: Add KID here
    trainer.extend(calc_KID(G_tgt, out, act_file=config['eval']['activations'] ,n_ims=5000, path='datasets/inception_model', num_evals=1),
                    #trigger=(10, 'iteration'), # KID test - wait only 10 iters to compute
                    trigger=(config.evaluation_interval, 'iteration'),
                    priority=extension.PRIORITY_WRITER)

    ## Plot loss graph
    trainer.extend(extensions.PlotReport(["loss_dis", "loss_gen"], x_key ='iteration', file_name='loss_graph.png'))

    # Plot FID graph
    trainer.extend(extensions.PlotReport(["FID_mean"], x_key ='iteration', file_name='FID_graph.png'))
    # Plot KID graph
    trainer.extend(extensions.PlotReport(["KID_mean"], x_key ='iteration', file_name='KID_graph.png'))

    # Save every 500 iterations
    #trainer.extend(extensions.snapshot(filename='snapshot_iter-{.updater.iteration}'))
